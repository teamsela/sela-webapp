...
export const RootBlock = ({
  ...
  useEffect(() => {
    ...
    const isSameColorFill = descendants.every((x) => {
      if (ctxExpandedStrophes[Number(x.stropheId)] === false || ctxExpandedStanzas[Number(x.stanzaId)] === false ) {
        return true;
      }
      else return x.colorFill === colorFill;
    });
    if (!isSameColorFill) {
    ...
    const isSameTextColor = descendants.every((x) => {
      if (ctxExpandedStrophes[Number(x.stropheId)] === false || ctxExpandedStanzas[Number(x.stanzaId)] === false ) {
        return true;
      }
      else return x.textColor === textColor;
    });
    if (!isSameTextColor) {
