...
export const findFloorIndex = (arr: number[], num: number) =>  {
        let idx = -1;
...
// export const mapByRanges = (reference: StropheNote[], lookup: StropheNote[]) => {
//   // Sort reference array by firstWordId to ensure ordering
//   const sortedRef = [...reference].sort((a, b) => a.firstWordId - b.firstWordId);

//   return lookup.map(item => {
//     const targetId = item.firstWordId;

//     for (let i = 0; i < sortedRef.length; i++) {
//       const current = sortedRef[i];
//       const next = sortedRef[i + 1];

//       // Case 1: exact match
//       if (current.firstWordId === targetId) return current;

//       // Case 2: in between current and next
//       if (next && targetId > current.firstWordId && targetId < next.firstWordId) {
//         return current;
//       }
//     }

//     // If no match, return null or original item
//     return null;
//   });
// }
...
// export const findPosition = (num: number, arr2: number[]): number =>  {
//   return arr2.findIndex((val, i) =>
//     num >= val && (i === arr2.length - 1 || num < arr2[i + 1])
//   );
// }

// export const findDuplicateIndexes = (arr: number[]): Record<number, number[]> => {
