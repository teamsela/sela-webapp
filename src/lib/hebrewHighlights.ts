...
/* ------------------------------------------------------------------ */
/*  WLC Hebrew → OHB transliteration (with prefixes & vowels)         */
/* ------------------------------------------------------------------ */

/**
 * Convert a WLC Hebrew word (with nikkud) to OHB-style dot-separated transliteration.
 * e.g. "לְדָוִד" → "le.da.vid", "מִזְמוֹר" → "miz.mor"
 */
export const transliterateHebrew = (wlcWord: string): string => {
  if (!wlcWord) return "";

  // Parse into clusters: each cluster = consonant + its combining marks
  type Cluster = { base: string; hasDagesh: boolean; hasShinDot: boolean; hasSinDot: boolean; vowel: string };
  ...
  for (const char of wlcWord) {
    ...
    // Dagesh/Mapiq U+05BC
    if (cp === 0x05BC) { current.hasDagesh = true; continue; }
    // Shin dot U+05C1
    if (cp === 0x05C1) { current.hasShinDot = true; continue; }
    // Sin dot U+05C2
    if (cp === 0x05C2) { current.hasSinDot = true; continue; }
  ...
  for (let i = 0; i < clusters.length; i++) {
    const cl = clusters[i];
    const next = i < clusters.length - 1 ? clusters[i + 1] : null;
    ...
    if (cl.base === "ש") {
      ...
      consonant = simpleMap[cl.base] ?? "";
    }

    // Special: vav + cholam/shuruq → vowel letter, not consonant "v"
    if (cl.base === "ו") {
      if (cl.vowel === "o") { // cholam male (וֹ)
        syllable += "o";
        syllables.push(syllable);
        syllable = "";
        continue;
      }
      if (cl.hasDagesh && !cl.vowel) { // shuruq (וּ)
        syllable += "u";
        syllables.push(syllable);
        syllable = "";
        continue;
    ...
    if (cl.vowel) {
      syllable += cl.vowel;
      // Shva (mobile) — produces a very short vowel, often starts a new syllable
      const isShva = cl.vowel === "e" && !cl.hasDagesh;
      const isWordInitialShva = i === 0 && isShva;

      if (isWordInitialShva) {
        // Word-initial shva is always mobile → separate syllable
        syllables.push(syllable);
        syllable = "";
      } else if (isShva && i > 0) {
        // Mid-word shva: close current syllable
        syllables.push(syllable);
        syllable = "";
      } else {
        // Regular vowel: close syllable
        syllables.push(syllable);
        syllable = "";
      }
    }
